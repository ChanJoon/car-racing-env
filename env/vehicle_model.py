...
class VehicleModel:
    ...
    def f_mpcc(self, X, Y, psi, vx, vy, omega, theta, delta, D, vtheta, ddelta, dD, dvtheta):
        '''
        Contunuons-time dynamics model for MPCC.
        Arguments
        ---------
        X: X-axis position of the Vehicle on the global coordinates.
        Y: Y-axis position of the Vehicle on the global coordinates.
        ...
        delta: Steering angle.
        D: Duty-cycle of the PWM signal of the DC motor.
        vtheta: Virtual velocity which propagates the reference path.
        ddelta: Steering angle change rate.
        dD: Duty-cycle change rate.
        dvtheta: Virtual velocity change rate.
        '''

        dx, dy, dpsi, dvx, dvy, domega = self.f(X, Y, psi, vx, vy, omega, delta, D)
        dtheta = vtheta

        return dx, dy, dpsi, dvx, dvy, domega, dtheta, ddelta, dD, dvtheta


    def f_pp(self, theta, ec, epsi, vx, vy, omega, delta, D, ddelta, dD, kapparef):
        '''
